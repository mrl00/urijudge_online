...
avg3 v@(a:as) w@(b:bs) = sum (map (\(x,y)-> x*y) $ zip v w) / sum w

scoreStatus :: Double -> String
scoreStatus n | n >= 7.0 = "Aluno aprovado.\n"
...
inExam m = do
   ...
   where inExSS k | k >= 5.0 = scoreStatus 7.0
                  | otherwise = scoreStatus 0.0

studentStatus :: Double -> IO ()
studentStatus m | m >= 7.0 = printf (fmt ++ scoreStatus m) m
